-- ═══════════════════════════════════════════════════════════
--  CLUB ASYMPTOTES — CORE TEAM PORTAL · COMPLETE ONE-SHOT SETUP
--
--  Run this WHOLE file ONCE in:
--    Supabase Dashboard → (your project) → SQL Editor → New query
--    → paste everything → Run
--
--  It is fully idempotent — safe to run again if unsure. It combines
--  migrations 25 + 26 + 27: creates every table, seeds the 25 core
--  members with their access codes, and links anyone who already has
--  a student account.
-- ═══════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.core_members (
  ...
  tier         TEXT        NOT NULL DEFAULT 'member' CHECK (tier IN ('council','head','member')),
...
CREATE TABLE IF NOT EXISTS public.core_feedback (
  ...
  team_id         UUID        REFERENCES public.core_teams(id) ON DELETE SET NULL,
  kind            TEXT        NOT NULL DEFAULT 'suggestion' CHECK (kind IN ('suggestion','complaint')),
  body            TEXT        NOT NULL,
...
-- ── RLS off (matches the data-plane convention) ───────────
...
INSERT INTO public.core_members (name, email, team_id, position, tier, access_code) VALUES
  ...
  ('Atul Dhull',                 '24ug1byai146@bmsit.in',  NULL, 'Vice-President',  'council', 'ASYM-AD2K'),
...
-- ── LINK members who already have a student account (migration 27) ──
...
-- ── VERIFY — should show 25 members, then the full code list ──
SELECT count(*) AS core_members_count FROM public.core_members;

SELECT name, access_code,
       CASE WHEN user_id IS NULL THEN 'needs to redeem code' ELSE 'already linked' END AS status
